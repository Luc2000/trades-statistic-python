...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('stock_collection_v2.log'),
...
SYMBOL_ALIASES = {
    "TRPL4": ["TRPL4", "TRPL3", "ISA"],
    "VBBR3": ["VBBR3", "BRDT3"],  # Antigo nome da Vibra Energia
    "BRPR3": ["BRPR3", "BRPR11"],
    "SOMA3": ["SOMA3"],
    "SQIA3": ["SQIA3"],
}
...
def collect_stock_data(symbol, start_date=None, max_retries=3):
    """
    Coleta dados de um símbolo com retry e backoff exponencial usando investpy.
    """
    ...
    while retry < max_retries and script_running:
        try:
            ...
            # Formata datas para o formato esperado pelo investpy (dd/mm/yyyy)
            from_date = start_date.strftime('%d/%m/%Y')
            to_date = end_date.strftime('%d/%m/%Y')
            
            # Remove qualquer sufixo como .SA que possa ter
            search_symbol = symbol
            if search_symbol.endswith('.SA'):
                search_symbol = search_symbol[:-3]
            
            # Tenta buscar dados da bolsa brasileira
            try:
                ...
                if not stock_info.empty:
                    try:
                        name = stock_info.loc['Name', 'Value']
                        sector = stock_info.loc['Sector', 'Value'] if 'Sector' in stock_info.index else 'N/A'
                        logging.info(f"Dados do ativo: {symbol} - {name}")
                        logging.info(f"Setor: {sector}")
                    except Exception as info_err:
                        logging.warning(f"Não foi possível extrair informações adicionais para {symbol}: {str(info_err)}")
                ...
                logging.warning(f"Erro na tentativa direta com {search_symbol}: {str(inner_err)}")
                
                # Tenta com search para encontrar o símbolo correto
                try:
                    search_results = investpy.search_stocks(by='symbol', value=search_symbol)
                    if not search_results.empty:
                        # Filtra resultados para Brasil
                        brazilian_stocks = search_results[search_results['country'] == 'brazil']
                        if not brazilian_stocks.empty:
                            # Usa o primeiro resultado
                            first_match = brazilian_stocks.iloc[0]
                            exact_symbol = first_match['symbol']
                            name = first_match['name']
                            
                            logging.info(f"Símbolo encontrado via busca: {exact_symbol} - {name}")
                            
                            # Tenta obter dados com o símbolo exato
                            df = investpy.get_stock_historical_data(
                                stock=exact_symbol,
                                country='brazil',
                                from_date=from_date,
                            ...
                            if not df.empty:
                                logging.info(f"Dados obtidos via busca: {len(df)} registros")
                                return name, df
                except Exception as search_err:
                    logging.warning(f"Erro na busca para {search_symbol}: {str(search_err)}")
...
def main():
    ...
    logging.info(f"Usando investpy para coleta de dados")
